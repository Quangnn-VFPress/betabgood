...
class Core_View_Helper_ItemPhoto extends Engine_View_Helper_HtmlImage
{
  ...
  public function itemPhoto($item, $type = 'thumb.profile', $alt = "", $attribs = array())
  {
    ...
    // Values
    $gender = '2';
    $values = Engine_Api::_()->fields()->getFieldsValuesByAlias($item);
    //print_r($values);
    ...
    $attribs['class'] .= 'item_photo_' . $item->getType() . '_'.$gender.' ';
    ...
    {
      $src = $this->getNoPhoto($item, $safeName, $gender);
      //echo $gender.'---'.$src;
      //$src = $value;
  ...
  public function getNoPhoto($item, $type, $gender)
  {
